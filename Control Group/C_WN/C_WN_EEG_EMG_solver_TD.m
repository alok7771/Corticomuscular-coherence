function [f,t,cl,sc,fname,pool_name,offset]= C_WN_EEG_EMG_solver_TD(EEG,EMG,EEG_ch_no,EMG_ch_no,triggers,leg,imagedir,vfigdir,vpngdir,num)

%% this solver solves for f,t,cl,sc and pass that to the time dependent
%plot routine. plots are saved to the directory generated by parent program
%named as "C_WN_autocode_TD".
...
namedir='C:\Users\Alok Ranjan\Dropbox\BME coursework\2b\Internship\Data Alok\Working_directory\My codes\Control Group\C_WN\';
...
saveas(h,fullfile(strcat(namedir,char(imagedir),'\',vpngdir),strcat('C_WN_',fig_name,'_1','.png')));
saveas(h,fullfile(strcat(namedir,char(imagedir),'\',vfigdir),strcat('C_WN_',fig_name,'_1','.fig')));
saveas(hh,fullfile(strcat(namedir,char(imagedir),'\',vpngdir),strcat('C_WN_',fig_name,'_2','.png')));
saveas(hh,fullfile(strcat(namedir,char(imagedir),'\',vfigdir),strcat('C_WN_',fig_name,'_2','.fig')));
% saveas(hhh,fullfile(strcat(namedir,char(imagedir),'\',vpngdir),strcat('C_WN_',fig_name,'_3','.png')));
% saveas(hhh,fullfile(strcat(namedir,char(imagedir),'\',vfigdir),strcat('C_WN_',fig_name,'_3','.fig')));
close all
end
