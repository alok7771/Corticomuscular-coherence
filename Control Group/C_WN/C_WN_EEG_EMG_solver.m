function [f,t,cl,sc,fname,pool_name]= C_WN_EEG_EMG_solver(EEG,EMG,EEG_ch_no,EMG_ch_no,triggers,leg,k,imagedir,vfigdir,vpngdir,num)
%% this solver solves for f,t,cl,sc and pass that to the time independent
%plot routine. plots are saved to the directory generated by parent program
%named as "C_WN_autocode".
...
% fig_number=strcat('C_WN_',fig_name);
...
saveas(h,fullfile(strcat(namedir,char(imagedir),'\',char(vpngdir)),strcat('C_WN_',fig_name,'.png')));
saveas(h,fullfile(strcat(namedir,char(imagedir),'\',char(vfigdir)),strcat('C_WN_',fig_name,'.fig')));
close 
