function C_WEA_autocode_pooled(EEG_ch_no,EMG_ch_no,leg)
...
if (ind==1)
    ...
    [plf1,plv1]=pool_scf(SC(:,:),CL(:));
  else
    % Pass pooled variables as arguments for further sets.
    [plf1,plv1]=pool_scf(SC(:,:),CL(:),plf1,plv1);
  end
end
...
% saveas(h,fullfile(sfpath,sprintf('pooled_analysis_T_IND.png')));
saveas(h,fullfile(sfpath,strcat('C_WEA_',pool_name,'.png')));
saveas(h,fullfile(sfpath,strcat('C_WEA_',pool_name,'.fig')));
close;
end