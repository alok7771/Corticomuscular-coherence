function [f,t,cl,sc,fname,pool_name,offset]= C_WEA_EEG_EMG_solver_TD(EEG,EMG,EEG_ch_no,EMG_ch_no,triggers,leg,imagedir,vfigdir,vpngdir,num)
...
[h,hh]= psp2_tf_alok(f,t,cl,freq,lag_tot,lag_neg,t_inc,n_contour,line_flag); %% for TD plot
saveas(h,fullfile(strcat(namedir,char(imagedir),'\',vpngdir),strcat('C_WEA_',fig_name,'_1','.png')));
saveas(h,fullfile(strcat(namedir,char(imagedir),'\',vfigdir),strcat('C_WEA_',fig_name,'_1','.fig')));
saveas(hh,fullfile(strcat(namedir,char(imagedir),'\',vpngdir),strcat('C_WEA_',fig_name,'_2','.png')));
saveas(hh,fullfile(strcat(namedir,char(imagedir),'\',vfigdir),strcat('C_WEA_',fig_name,'_2','.fig')));
