function [f,t,cl,sc,fname,pool_name,offset]= PD_WEA_EEG_EMG_solver_TD(EEG,EMG,EEG_ch_no,EMG_ch_no,triggers,leg,imagedir,vfigdir,vpngdir,num)
...
saveas(h,fullfile(strcat(namedir,char(imagedir),'\',vpngdir),strcat('PD_WEA_',fig_name,'_1','.png')));
saveas(h,fullfile(strcat(namedir,char(imagedir),'\',vfigdir),strcat('PD_WEA_',fig_name,'_1','.fig')));
saveas(hh,fullfile(strcat(namedir,char(imagedir),'\',vpngdir),strcat('PD_WEA_',fig_name,'_2','.png')));
saveas(hh,fullfile(strcat(namedir,char(imagedir),'\',vfigdir),strcat('PD_WEA_',fig_name,'_2','.fig')));
close all
end
