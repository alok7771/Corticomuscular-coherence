function [f,t,cl,sc,fname,pool_name]= PD_WN_EEG_EMG_solver(EEG,EMG,EEG_ch_no,EMG_ch_no,triggers,leg,k,imagedir,vfigdir,vpngdir,num)
...
saveas(h,fullfile(strcat(namedir,char(imagedir),'\',char(vpngdir)),strcat('PD_WN_',fig_name,'.png')));
saveas(h,fullfile(strcat(namedir,char(imagedir),'\',char(vfigdir)),strcat('PD_WN_',fig_name,'.fig')));
close 
